...
class AudioDetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (embeddedPicture != null) {
            ...
            audio_album_art_iv.setImageBitmap(bitmap)
        } else
            audio_album_art_iv.setImageResource(R.drawable.unknow_audio)
    }

    fun playPauseMusic() {
        if (mp.isPlaying) {
            mp.pause()
            audio_detail_play_pause_ib.setImageResource(android.R.drawable.ic_media_play)
        } else {
            mp.start()
            audio_detail_play_pause_ib.setImageResource(android.R.drawable.ic_media_pause)
            performViewUpdates()
        }
    }
